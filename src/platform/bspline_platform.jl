...
primal_bspline_generator(::Type{T}, degree::NTuple{N,Int}) where {N,T} = BasisFunctions.tensor_generator(T, map(d->primal_bspline_generator(T, d), degree)...)
...
bspline_param(init::NTuple{N,Int}) where N = BasisFunctions.TensorSequence([BasisFunctions.MultiplySequence(i,2 .^(1/length(init))) for i in init])
