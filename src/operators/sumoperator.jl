...
struct OperatorSum{T,S,N} <: DictionaryOperator{T}
    ...
    function OperatorSum{T}(s, d, ops, coefficients, scratch) where {T}
        ...
        new{T,typeof(scratch),length(ops)}(s, d, ops, coefficients, scratch)
    end
end

OperatorSum(s, d, ops, coefficients; T = promote_type(map(eltype, ops)...)) = 
    OperatorSum{T}(s, d, ops, [T(c) for c in coefficients], zeros(T, dest(ops[1])))
...
function apply!(op::OperatorSum{S,T,N}, coef_dest, coef_src) where {S,T,N}
    coef_dest[:] .= 0
