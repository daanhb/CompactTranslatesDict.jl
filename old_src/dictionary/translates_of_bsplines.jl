...
interpolation_grid(b::DiffPeriodicBSplineBasis) = isodd(degree(b)) ? PeriodicEquispacedGrid(length(b), support(b)) : MidpointEquispacedGrid(length(b), support(b))
...
"""
struct BSplineTranslatesBasis{T,K,SCALED} <: PeriodicBSplineBasis{K,T}
    n               :: Int
end
...
function primalgramcolumnelement(s::BSplineTranslatesBasis{T,K,SCALED}, i::Int; options...) where {T,K,SCALED}
    r = 0
    A = coefficienttype(s)
